...
namespace Jida\Trash\Manager\Rutas;

use Jida\Configuracion\Config;
use Jida\Core\Manager as Core;
use Jida\Trash\Manager\Estructura;
use Jida\Trash\Manager\Excepcion;
use Jida\Trash\Manager\Rutas\Procesador\Url;
use Jida\Trash\Manager\Vista\Data;
use Jida\Trash\Manager\Vista\Manager as ManagerVista;
...
class Arranque {
     ...
     * @see \Jida\Trash\Manager\Vista\Manager
     */
    ...
    private function pipeLectura() {

        foreach ($this->_handlers as $handler) {

            $handler = new $handler($this->url);
            $handler->validacion();

            if ($handler::$aplica) {
                $this->_handlersActivos = $handler;
            }
    ...
    private function _parametros($parametros) {
        ...
        if (isset($parametros['handlers'])) {
            ...
            if (is_string($parametros['handlers'])) $parametros['handlers'] = (array)$parametros['handlers'];
            $this->_handlers = $parametros['handlers'];
        }
